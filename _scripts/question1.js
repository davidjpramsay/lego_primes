function buildQ1(factorsA, factorsB, x1, x2, factorsQ1) {
    ...
    // Render the prime factors for the least common multiple.
    let i = 0;
    for (const primeN in factorsQ1) {
        let j = 0;
        while (j < factorsQ1[primeN]) {
            svg.append("rect")
                .attr(
                    "x",
                    i * legoWidth + ((svgWidth) - legoWidth * Object.keys(factorsQ1).length) / 2
                )
            ...
            svg.append("text")
                .attr(
                    "x",
                    i * legoWidth + ((svgWidth) - legoWidth * Object.keys(factorsQ1).length) / 2 + legoWidth / 2
                )
